...
long long main_theme(std::vector<std::pair<long long, long long>>& v, long long hash_x, long long x, long long hash_y, long long y) {
    ...
    std::swap(v[temp_y].second, v[temp_x].second);
    return std::abs(v[temp_y].second - v[temp_x].second);
}
int main()
{
    ...
    std::cout.tie(NULL);
    long long m = 400000;
    std::vector<std::pair<long long, long long>> v(m, { pow(10,10),0 });
    long long n;
    std::cin >> n;
    ...
    for (long long i = 0; i < n; ++i) {
        ...
        std::cout << main_theme(v, hash_x, x, hash_y, y) << '\n';
    }
    return 0;
}
